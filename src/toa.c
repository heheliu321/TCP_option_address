...
init_toa_ip6(void)
{
    ...
    if (toa_ip6_sk_lock.lock == NULL) {
        TOA_INFO("fail to alloc toa ip6 percpu lock\n");
        return -ENOMEM;
    }
            ...
            switch (opcode) {
                ...
                default:
                    ...
                    if (TCPOPT_TOA == opcode &&
                        ...
                        /* Because the size of ret_ptr is exactly the same as
                         *  struct toa_ip4_data(8 bytes), we can just copy it.
                         */
                        memcpy(&ret_ptr, ptr - 2, sizeof(struct toa_ip4_data));
                        TOA_DBG("af = %d, find toa data: ip = "
                                TOA_NIPQUAD_FMT", port = %u\n",
                                af,
                                TOA_NIPQUAD(((struct toa_ip4_data *)&ret_ptr)->ip),
                                ntohs(((struct toa_ip4_data *)&ret_ptr)->port));
#ifdef TOA_IPV6_ENABLE
                        if (af == AF_INET6) {
                            ...
                            ipv6_addr_set(&ptr_toa_ip6->in6_addr, 0, 0,
                                    htonl(0x0000FFFF), ((struct toa_ip4_data *)&ret_ptr)->ip);
                            ptr_toa_ip6->port = ((struct toa_ip4_data *)&ret_ptr)->port;
...
#endif
{
    ...
    /* call orginal one */
#if LINUX_VERSION_CODE < KERNEL_VERSION(4,17,0)
    /* inet_getname return >=0 after 4.16 */
...
{
#ifndef __aarch64__
    /* no need to modify the right for physical page in arm platform */
